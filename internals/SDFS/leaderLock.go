...
func requestLock(requestorAddress string, fileName string, requestType RequestType) {
	...
	for !canProceed {
		switch requestType {
		case READ:
			canProceed = lock.writeCount == 0 && (lock.readCount < 2 || lock.consecutiveWrites >= 4) && lock.readQueue[0] == requestorAddress
		case WRITE:
			canProceed = lock.writeCount == 0 && lock.readCount == 0 && (lock.consecutiveReads >= 4 || lock.readCount == 0) && lock.writeQueue[0] == requestorAddress
		}
		if !canProceed {
